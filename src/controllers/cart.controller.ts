...
const cartController = {
  createCart: async (req: IRequest, res: IResponse, next: INextFunction) => {
    try {
      ...
      let getCart = await CartService.getCart({
        product: req.body.product_id,
        size: req.body.size,
        user: req.decoded.id,
      });
      ...
      if (
        getCart &&
        getCart.product.toString() === req.body.product_id &&
        getCart.size === req.body.size
      ) {
        // updated quantity
        ...
        if (!updateCart) {
          return res
            ...
            .send({
              status: CART_RESPONSE.FAILED,
              message: CART_RESPONSE.CART_DOESNT_UPDATED,
            });
        }
        res.send({
          status: CART_RESPONSE.SUCCESS,
          message: CART_RESPONSE.CART_UPDATED,
          data: updateCart,
        });
      } else {
        // creaCart
        ...
        if (!createCart) {
          res
            ...
            .send({
              status: CART_RESPONSE.FAILED,
              message: CART_RESPONSE.CART_DOESNT_CREATED,
            });
        }
        res.send({
          status: CART_RESPONSE.SUCCESS,
          message: CART_RESPONSE.CART_CREATED,
          data: createCart,
        });
      }
    } catch (error) {
    ...
    try {
      ...
      if (!getManyCart) {
        return res
          ...
          .send({
            status: CART_RESPONSE.FAILED,
            message: CART_RESPONSE.GET_ALL_DOESNT_CART,
          });
      }
      res.send({
        status: CART_RESPONSE.SUCCESS,
        message: CART_RESPONSE.GET_ALL_CART,
        data: getManyCart,
      });
    } catch (error) {
  ...
  getCart: async (req: IRequest, res: IResponse, next: INextFunction) => {
    try {
      ...
      if (!getCart) {
        return res.send({
          status: CART_RESPONSE.FAILED,
          message: CART_RESPONSE.GET_DOESNT_CART,
          data: {},
        });
      }
      ...
      if (getCart.status === CART.QUEUED) {
        res.send({
          status: CART_RESPONSE.SUCCESS,
          message: CART_RESPONSE.GET_CART,
          data: getCart,
        });
      } else {
        res.send({
  ...
  deleteCartById: async (
    ...
    try {
      ...
      res.send({
        status: CART_RESPONSE.SUCCESS,
        message: CART_RESPONSE.DELETE_CART,
        data: deleteCartById,
      });
    } catch (error) {
      error.desc = CART_RESPONSE.GET_ALL_DOESNT_CART;
