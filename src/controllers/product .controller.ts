...
const ProductController={
    ...
    createProduct:async(req:IRequest,res:IResponse,next:INextFunction)=>{
        ...
        if(!createProduct){
          res.status(HTTP.UNPROCESSABLE_ENTITY).send({status:PRODUCT_RESPONSE.FAILED,message:PRODUCT_RESPONSE.PRODUCT_DOESNT_CREATED})
        }
       
        res.send({status:PRODUCT_RESPONSE.SUCCESS,message:PRODUCT_RESPONSE.PRODUCT_CREATED,data:createProduct})
    } catch (error) {
    ...
    getAllProduct:async(req:IRequest,res:IResponse,next:INextFunction)=>{
        try {
          const {categories_search,brand_search,color_search,size_search,price_search} =req.body
          let query:any={}
         ...
         if(price_search&& price_search.length>0){
          query.price={$regex:price_search,$options:"i"}
            ...
            const getProduct:any= await ProductService.getAllProduct(query)
            if(!getProduct){
                res.status(HTTP.UNPROCESSABLE_ENTITY).send({status:PRODUCT_RESPONSE.FAILED,message:PRODUCT_RESPONSE.GET_DOESNT_PRODUCT})
              }
             
              res.send({status:PRODUCT_RESPONSE.SUCCESS,message:PRODUCT_RESPONSE.GET_PRODUCT ,data:getProduct})
            } catch (error) {
