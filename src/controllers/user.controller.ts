    ...
    try {
      ...
      if (!token)
        return res
          ...
          .send({ auth: false, message: USER_RESPONSE.NO_TOKEN_PROVIDED });

      if (!token.includes("Bearer "))
        return res
          .status(403)
          .send({ auth: false, message: USER_RESPONSE.INVALID_TOKEN });
      ...
      if (decoded) {
        ...
        if (user) {
          ...
          return res
            .status(HTTP.UNAUTHORIZED)
            .send({ auth: false, message: USER_RESPONSE.TOKEN_ERROR });
        }
      } else {
        return res.status(HTTP.UNAUTHORIZED).send({
          auth: false,
          message: USER_RESPONSE.TOKEN_USER_DOESNT_EXIST,
        });
      }
    } catch (err) {
  ...
  userSignup: async (req: IRequest, res: IResponse, next: INextFunction) => {
    try {
      ...
      if (!user) {
        ...
        // status update
        res.send({
          status: USER_RESPONSE.SUCCESS,
          message: USER_RESPONSE.USER_CREATED,
          data: user,
          token: token,
        });
      } else if (user && req.body.email_verified) {
        let session_token = generateSessionToken();
        if (user) {
          ...
          res.send({
            status: USER_RESPONSE.SUCCESS,
            message: USER_RESPONSE.USER_EXIST,
            data: user,
            token,
            role: user.role,
          });
        ...
        if (update) {
          ...
          res.status(HTTP.UNPROCESSABLE_ENTITY).send({
            status: USER_RESPONSE.FAILED,
            message: USER_RESPONSE.SIGNUP_FAILED,
          });
        }
      } else {
        res.status(HTTP.UNPROCESSABLE_ENTITY).send({
          status: USER_RESPONSE.FAILED,
          message: USER_RESPONSE.EMAIL_EXISTS,
        });
      }
    } catch (error) {
      ...
      if (user) {
        ...
        if (isTrue) {
          ...
          res.status(HTTP.UNAUTHORIZED).send({
            status: USER_RESPONSE.FAILED,
            message: USER_RESPONSE.INCORRECT_PASSWORD,
          });
        }
      } else {
        res.status(HTTP.UNAUTHORIZED).send({
          status: USER_RESPONSE.FAILED,
          message: USER_RESPONSE.USER_DOESNT_EXIST,
        });
      }
    } catch (error) {
    ...
    try {
      ...
      if (!getManyUser) {
        return res.status(HTTP.UNPROCESSABLE_ENTITY).send({
          status: USER_RESPONSE.FAILED,
          message: USER_RESPONSE.GET_ALL_DOESNT_USER,
        });
      }
      res.send({
        status: USER_RESPONSE.SUCCESS,
        message: USER_RESPONSE.GET_ALL_USER,
        data: getManyUser,
        count: getManyUser.length,
      });
    } catch (error) {
  ...
  getUser: async (req: IRequest, res: IResponse, next: INextFunction) => {
    try {
      ...
      if (!getUser) {
        return res.send({
          status: USER_RESPONSE.FAILED,
          message: USER_RESPONSE.GET_DOESNT_USER,
        });
      }
      res.send({
        status: USER_RESPONSE.SUCCESS,
        message: USER_RESPONSE.GET_USER,
        data: getUser,
      });
    } catch (error) {
